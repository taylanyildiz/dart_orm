...
cornerFinder(cv::Mat &myMat, int answerCount, int questionCount)
{
    ...
    cv::findContours(matCanny, contours, hierarchy, cv::RETR_EXTERNAL, cv::CHAIN_APPROX_NONE);
    ...
    {
        // platform_log("insufficient contour amount - not enough rectangles: %d", rectCon.size());
        return std::vector<cv::Point>();
    }
    ...
    // //    DRAWING CONTOURS
    //    cv::drawContours(myMat, std::vector<std::vector<cv::Point>>{squarecorners[0]}, -1, cv::Scalar(255, 0, 0), 2);
    //    cv::drawContours(myMat, std::vector<std::vector<cv::Point>>{squarecorners[1]}, -1, cv::Scalar(255, 0, 255), 2);
    //    cv::drawContours(myMat, std::vector<std::vector<cv::Point>>{squarecorners[2]}, -1, cv::Scalar(0, 0, 255), 2);
    //    cv::drawContours(myMat, std::vector<std::vector<cv::Point>>{squarecorners[3]}, -1, cv::Scalar(0, 255, 255), 2);
    //    cv::drawContours(myMat, std::vector<std::vector<cv::Point>>{squarecorners[4]}, -1, cv::Scalar(0, 255, 0), 2);
    //    if (squareCount == 6) cv::drawContours(myMat, std::vector<std::vector<cv::Point>>{squarecorners[5]}, -1, cv::Scalar(255, 255, 0), 2);
...
extern "C"
{
    ...
    __attribute__((visibility("default"))) __attribute__((used)) void readOpticalForm(char *inputImagePath, char *outputPath, int questionCount, int answerCount, int *resultBuffer)
    {
        ...
        //    CORNER FINDING
        std::vector<cv::Point> biggestContour = cornerFinder(resized, answerCount, questionCount);
        ...
        cv::warpPerspective(matBiggestContours, matWarpColored, matrix, cv::Size(450, 750));
        ...
        std::vector<std::vector<int>> pixelValues(questionCount, std::vector<int>(answerCount));
        int countC = 0;
        int countR = 0;
        int boxesTotal = 0;
...
splitBoxes(const cv::Mat &image, int questionCount, int choiceCount)
{
    ...
    if (questionCount <= 10)
    {
        ...
        for (int r = 0; r < numRows; ++r)
        {
            ...
            {
                if ((r == 0 && c == 0) || (r == 0 && c == choiceCount + 1) || (r == questionCount + 5 && c == 0) || (r == questionCount + 5 && c == choiceCount + 1))
                {
    ...
    else if (questionCount > 10 && questionCount <= 20)
    {
        ...
        for (int r = 0; r < numRows; ++r) // first half
        {
            for (int c = 0; c < numCols; ++c)
            {
                if ((r == 0 && c == 0) || (r == 0 && c == choiceCount * 2 + 3) || (r == 10 + 5 && c == 0) || (r == 10 + 5 && c == choiceCount * 2 + 3))
                {
                    cv::Mat roi = getBox(image, boxWidth, boxHeight, c, r);
                    int totalPixels = cv::countNonZero(roi);
                    platform_log(">>>>>>totalPixels:%i allpixels:%i r:%i c:%i \n", totalPixels, roi.total(), r, c);
        ...
        for (int r = 0; r < numRows; ++r) // secondhalf
        {
            for (int c = 0; c < numCols; ++c)
            {
                if (r >= 5 && r < 5 + questionCount - 10 && c >= choiceCount + 4 && c < 4 + choiceCount * 2)
                {
                    cv::Mat roi = getBox(image, boxWidth, boxHeight, c, r);
                    boxes.push_back(roi);
                }
    ...
    else
    {
        ...
        for (int r = 0; r < numRows; ++r) // first half
        {
            for (int c = 0; c < numCols; ++c)
            {
                if ((r == 0 && c == 0) || (r == 0 && c == numCols - 1) || (r == numRows - 1 && c == 0) || (r == numRows - 1 && c == numCols - 1))
                {
                    cv::Mat roi = getBox(image, boxWidth, boxHeight, c, r);
                    int totalPixels = cv::countNonZero(roi);
                    platform_log(">>>>>>totalPixels:%i allpixels:%i r:%i c:%i \n", totalPixels, roi.total(), r, c);
                }
                if (r >= 5 && r < 5 + questionCount / 2 + questionCount % 2 && c >= 2 && c < 2 + choiceCount)
                {
                    cv::Mat roi = getBox(image, boxWidth, boxHeight, c, r);
                    boxes.push_back(roi);
                }
        ...
        for (int r = 0; r < numRows; ++r) // secondhalf
        {
            for (int c = 0; c < numCols; ++c)
            {
                if (r >= 5 && r < 5 + questionCount / 2 && c >= choiceCount + 4 && c < 4 + choiceCount * 2)
                {
                    cv::Mat roi = getBox(image, boxWidth, boxHeight, c, r);
                    boxes.push_back(roi);
                }
...
getBox(cv::Mat image, float boxWidth, float boxHeight, int c, int r)
{
    ...
    if (roiRect.x < 0 || roiRect.y < 0 || roiRect.x + roiRect.width > image.cols || roiRect.y + roiRect.height > image.rows)
    {
        platform_log("ROI rect is out of bounds!    roiRect.x:%i roiRect.y:%i", roiRect.x, roiRect.y);
